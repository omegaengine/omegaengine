...
/// <summary>
/// Tests picking against a <see cref="Terrain"/>.
/// </summary>
/// <remarks>
/// Skips gracefully (rather than failing) where the terrain shaders cannot be provided, e.g. on machines that have
/// <c>d3dx9_43.dll</c> but not the <c>d3dcompiler_43.dll</c> that runtime HLSL compilation needs.
/// </remarks>
public class TerrainTest : EngineTestBase
{
    ...
    private Terrain CreateFlatTerrain(byte height = 50)
    {
        ...
        catch (Exception ex) when (IsShaderUnavailable(ex))
        {
            Assert.Skip($"No terrain shaders available: {ex.Message}");
            throw; // Unreachable (Assert.Skip throws), but proves all completing paths return a terrain
        }
    }

    /// <summary>
    /// Determines whether a failure was caused by the environment being unable to provide shaders rather than by a bug.
    /// </summary>
    private static bool IsShaderUnavailable(Exception ex)
        => ex switch
        {
