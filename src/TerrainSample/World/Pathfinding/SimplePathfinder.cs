...
namespace World.Pathfinding
{
    ...
    public class SimplePathfinder : IPathfinder<Vector2>
    {
        ...
        /// <inheritdoc/>
        public IEnumerable<Vector2> FindPathPlayer(Vector2 start, Vector2 end)
        {
            start.X = (int)start.X;
            start.Y = (int)start.Y;
            end.X = (int)end.X;
            ...
            using (new TimedLogEvent("Calculating path"))
            {
                ...
                _openList.Clear();
                _closeList.Clear();
                var parentNode = GetParentNode(start, end);
                ...
                while (_openList.Count != 0)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        nextNode.H = 10 * (int)(nextNode.Position - end).Length();
